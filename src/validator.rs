...
fn _validate<'a>(dst: &Dataset, file_path: &str) -> Result<bool, ValidateCOGError<'a>> {
...
fn _check_external_ovr<'a>(file_list: Vec<String>) -> Result<bool, ValidateCOGError<'a>> {
...
fn _check_main_band<'a>(band: &RasterBand, ovr_count: i32) -> Result<bool, ValidateCOGError<'a>> {
...
fn _validate_band<'a>(
    ...
    band_name: &'a str,
    band: &RasterBand,
) -> Result<bool, ValidateCOGError<'a>> {
    let block_size = band.block_size();
...
fn _check_trailer_bytes<'a>(
    ...
    if byte_count >= 4 {
        ...
        if left != right {
            return Err(ValidateCOGError::TrailerBytesError { band_name, x, y });
        }
    }
    Ok(true)
}

fn _validate_mask_band<'a>(
    f: &VSIFile,
...
fn _validate_ovr<'a>(
    ...
    for i in 0..ovr_count {
        ...
        let ovr = "overview";
        _validate_band(f, ovr, ovr_band)?;
        _validate_mask_band(f, ovr, ovr_band)?;
    }
    Ok(true)
}
